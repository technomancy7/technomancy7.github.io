...
<</if>>`;
    ...
    Macro.add('defdmg', {
        handler:function(){
            weapon_dmg[this.args[0]] = this.args[1];
    ...
    window.goals = function(){
	  return getv("goals", {"DEFAULT_GOAL": "No goal."});
    };
    ...
    Macro.add('goals', {
		...
		for (const key of Object.keys(goals)){
		    let item = goals[key];
		    if(item.active){ body += "<hr>"; body += `''${item.body}''`; } else completed += 1;
            }
		body += "<hr>";
    ...
    window.editNote = function(oldbody, newbody){
	  ...
	  for (var i = 0;i<=notes.length;i++){ if(notes[i] == oldbody){ notes[i] = newbody; break; } }
