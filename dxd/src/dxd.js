...
(function(){
    ...
    window.getv = function(key, defaultv = undefined){
     ...
     Macro.add('cli', {
	  handler: function(){
		let body = `<<textbox '$cli_input_tmp' '' autofocus>> ($cli_cwd)
		<<button 'Clear Log'>><<set $cli_log to null>><<closediag>><<cli>><</button>> | <<button 'Send'>><<set $cli_input to $cli_input_tmp>><<run cliLogCmd('$cli_input')>><<reload>><<closediag>><<cli>><</button>>
		<hr>
		<<if $cli_log isnot null>><<print $cli_log>><</if>>
		`;
		Dialog.close();
		Dialog.setup(`Command Line`);
            Dialog.wiki(body);
            Dialog.open();
	  }
     });
    ...
    Macro.add('parsecli', {
	  handler: function(){
		if(State.getVar("$cli_input") != undefined){
		    let cmds = State.getVar("$cli_input").split(" ");
		let cmd = cmds.shift();
		if(cmd == "goto"){
		    $(this.output).wiki(`<<goto '${cmds.join(" ")}'>>`);

		} else if(cmd == "cd"){
		    State.setVar("$cli_cwd", cmds.join(" "));
		}}

	  }
    });
    
    window.cliLogCmd = function(text){
	  let v = getv("cli_log", "");
	  text = State.getVar(text);
	  v = `${State.getVar("$cli_cwd")} ${text}\n${v}`;
	  State.setVar("$cli_log", v);
    };
    window.cliLog = function(text){
	  let v = getv("cli_log", "");
	  v = `${text}\n${v}`;
	  State.setVar("$cli_log", v);
    };
    ...
    window.inPuzzle = function(){ return Story.get(State.passage).tags.includes("puzzle"); };
    ...
    window.validateComputer = function(usr, usrinput, pwd, pwdinput){
	  console.log(`${State.getVar(usr)}-${usrinput} | ${State.getVar(pwd)}-${pwdinput}`);
    ...
    Macro.add('medicalbot', {
        handler: function(){
		let body = `
		<<showhealth>>
		<<if getPlayerHealth() <= 100>><<button>><<run giveHealth(null, 100)>><<medicalbot>><</button>><</if>>
		`;
		Dialog.setup("Medical Bot");
            Dialog.wiki(body);
            Dialog.open();
	  }
    });
    Macro.add('computer', {
	  tags: ['onpass', 'onfail'], 
		...
		let onfail = "<<qdiag 'Failed' 'Incorrect login details.'>>";
		if(this.payload[1] != undefined) onfail = this.payload[1].contents;
            let body = `\
            Username <<textbox "$computer_login" ''>>
            Password <<textbox "$computer_pass" ''>>
            <<button 'Confirm'>>
